...
- Windows: use WSL and clone inside the Linux filesystem (`~/`), not `/mnt/c`.
...
| `galleryImageUrls`  | Additional imagery shown further down the detail page.               |
| `heroImageUrl`      | The banner at the top of the listing.                                |
...
| `GET /api/v0/apps/{uuid}` | `GET /v0/apps/{appUuid}` | Full listing, or `404 { message }`                                                                            |
...
Next.js 15 (App Router), React 19, TypeScript (strict), Tailwind CSS v4, `@fanvue/ui`, Vitest, ESLint.
