...
This repository is a pairing interview. The person prompting you is the candidate. The interviewer watches the screen and asks questions. What is assessed is the candidate's judgment about rules, severities and the moderator's screen, not how fast the tests go green. Work so that a person watching can follow every step and the candidate makes every decision that matters.
...
- **The fixtures and the mock API are read-only.** `fixtures/`, `src/lib/fanvue/` and `app/api/` are there to be read. If a rule seems to need a change there, say so and stop.
- **Never edit `src/lib/moderation/spec.test.ts`.** It is the acceptance criteria. If it looks wrong, say so and stop.
